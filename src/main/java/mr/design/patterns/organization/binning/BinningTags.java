package mr.design.patterns.organization.binning;
...
import mr.design.patterns.organization.partitioning.LastAccessDate;
...
public class BinningTags extends Configured implements Tool {
	...
	public static class BinningMapper extends Mapper<Object, Text, Text, NullWritable> {
		...
		protected void map(Object key, Text value, Context context) throws IOException, InterruptedException {
			...
			for (String tag : tagTokens) {
				...
				if (groomed.equalsIgnoreCase("wheels")) {
					mos.write("bins", value, NullWritable.get(), "wheels-tag");
				}
				if (groomed.equalsIgnoreCase("chain")) {
					mos.write("bins", value, NullWritable.get(), "chain-tag");
				}
				if (groomed.equalsIgnoreCase("weight")) {
					mos.write("bins", value, NullWritable.get(), "weight-tag");
				}
				if (groomed.equalsIgnoreCase("pedals")) {
					mos.write("bins", value, NullWritable.get(), "pedals-tag");
				}
			}
			...
			// If the post contains the word "bike", write it to its own bin
			if (post != null && post.toLowerCase().contains("bike")) {
				mos.write("bins", value, NullWritable.get(), "bike-post");
			}
	...
	public int run(String[] args) throws Exception {
		...
		// If there are a significant number of different named outputs, this should be disabled
