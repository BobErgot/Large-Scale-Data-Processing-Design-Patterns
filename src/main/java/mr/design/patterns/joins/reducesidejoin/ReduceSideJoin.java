...
public class ReduceSideJoin extends Configured implements Tool {
    ...
    private void executeJoinLogic(Context context) throws IOException, InterruptedException {
      ...
      if (joinType.equalsIgnoreCase("inner")) {
        ...
        if (listA.isEmpty() ^ listB.isEmpty()) {
          ...
          // The previous XOR check will make sure exactly one of these lists is empty and
          // therefore the list will be skipped
          for (Text A : listA) {
            context.write(A, EMPTY_TEXT);
          }
          for (Text B : listB) {
  ...
  public int run(String[] args) throws Exception {
    ...
    if (otherArgs.length != 4) {
      System.err.println("Usage: ReduceSideJoin <user_in> <out> <comments_in> <join_type>");
    ...
    MultipleInputs.addInputPath(job, new Path(otherArgs[2]), TextInputFormat.class, CommentJoinMapper.class);
