package mr.design.patterns.joins.compositejoin;
...
public class CompositeUserJoin extends Configured implements Tool {
  ...
  public int run(String[] args) throws Exception {
    if (args.length != 4) {
      System.err.println("Usage: Composite Join <user_in> <out> <comments_in> [inner|outer]");
      ToolRunner.printGenericCommandUsage(System.err);
      System.exit(2);
    }
    Path userPath = new Path(args[0]);
    Path commentPath = new Path(args[2]);
  ...
  public static void main(String[] args) throws Exception {
    int res = ToolRunner.run(new CompositeUserJoin(), args);
    System.exit(res);
  }
}
