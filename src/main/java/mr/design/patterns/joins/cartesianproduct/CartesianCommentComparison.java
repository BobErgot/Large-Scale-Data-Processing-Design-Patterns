package mr.design.patterns.joins.cartesianproduct;
...
public class CartesianCommentComparison extends Configured implements Tool {
  ...
  public static class CartesianRecordReader<K1, V1, K2, V2> implements RecordReader<Text, Text> {
    ...
    private final FileInputFormat rightFIF;
    private final JobConf rightConf;
    private final InputSplit rightIS;
    private final Reporter rightReporter;
    // Helper variables
    private final K1 lkey;
    private final V1 lvalue;
    private final K2 rkey;
    private final V2 rvalue;
  ...
  public static class CartesianMapper extends MapReduceBase implements Mapper<Text, Text, Text, Text> {
    ...
    public void map(Text key, Text value, OutputCollector<Text, Text> output, Reporter reporter) throws IOException {
      ...
      if (!key.toString().equals(value.toString())) {
        Map<String, String> keyMap = transformXmlToMap(key.toString());
        Map<String, String> valueMap = transformXmlToMap(value.toString());
        if (keyMap.get("Text") == null || valueMap.get("Text") == null) return;
        String[] leftTokens = keyMap.get("Text").split("\\s");
        String[] rightTokens = valueMap.get("Text").split("\\s");
