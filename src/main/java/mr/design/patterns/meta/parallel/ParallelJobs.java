package mr.design.patterns.meta.parallel;
...
public class ParallelJobs {
  ...
  public static void main(String[] args) throws Exception {
    ...
    if (otherArgs.length != 3) {
      System.err.println("Usage: ParallelJobs <below-avg-in> <output> <above-avg-in>");
      System.exit(2);
    }

    Path belowAvgInputDir = new Path(otherArgs[0]);
    Path aboveAvgInputDir = new Path(otherArgs[2]);

    Path belowAvgOutputDir = new Path(otherArgs[1] + "/belowAverage");
    Path aboveAvgOutputDir = new Path(otherArgs[1] + "/aboveAverage");
